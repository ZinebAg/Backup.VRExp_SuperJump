...
function checkCollisions() {
    ...
    // Invulnerability check (milliseconds timeline)
    const now = performance.now();
    const invulnerable = (player.invulnerableUntil && now < player.invulnerableUntil);
    ...
    enemies.forEach(enemy => {
        ...
        if (checkCollision(playerCollisionBox, enemyCollisionBox)) {
            ...
            if (downward && fromAbove) {
                ...
                // Side/bottom contact: lose life (unless invulnerable)
                if (!invulnerable) {
                    loseLife(enemy);
                }
            }
    ...
    antennas.forEach(antenna => {
        ...
        if (checkCollision(playerCollisionBox, antennaCollisionBox)) {
            // Antennas are unkillable: any contact is fatal (no stomp)
            if (!invulnerable) {
                loseLife(antenna);
            }
        }
...
function render() {
    try {
        ...
        // Draw Heart UI (top-left, pinned to viewport)
